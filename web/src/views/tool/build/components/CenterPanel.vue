...
<template>
  <ScrollArea class="h-full">
    <div class="bg-muted/30 p-8 min-h-full" @click="selectField(null)">
      <div class="max-w-2xl mx-auto bg-background rounded-lg shadow-sm border min-h-[500px] p-6 space-y-4" @click.stop>
         <div class="text-center pb-4 border-b">
           <h3 class="text-lg font-medium">表单预览</h3>
           <p class="text-sm text-muted-foreground">点击组件配置属性，拖拽排序</p>
         </div>
         
         <div v-if="fields.length === 0" class="h-40 flex items-center justify-center text-muted-foreground border-2 border-dashed rounded-lg">
           请从左侧添加组件
         </div>
         ...
         <div 
            ...
            :class="{'ring-2 ring-primary border-primary': selectedFieldId === field.id, 'hover:border-primary': selectedFieldId !== field.id}"
            draggable="true"
         ...
         >
          <!-- Drag Handle -->
          <div class="absolute left-2 top-1/2 -translate-y-1/2 cursor-move opacity-0 group-hover:opacity-100 text-muted-foreground p-1 hover:text-foreground">
            <GripVertical class="h-4 w-4" />
          </div>

          <!-- Actions -->
          <div class="absolute right-2 top-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 backdrop-blur-sm rounded-md border shadow-sm p-0.5 z-10"
               :class="{'opacity-100': selectedFieldId === field.id}">
            <Button variant="ghost" size="icon" class="h-7 w-7" @click="(e: Event) => copyField(index, e)" title="复制">
              <Copy class="h-3.5 w-3.5" />
            </Button>
            <Button variant="ghost" size="icon" class="h-7 w-7 text-destructive hover:text-destructive" @click="(e: Event) => removeField(index, e)" title="删除">
          ...
          <div class="pl-6 pr-8 pointer-events-none">
             ...
             <template v-else>
              <Label class="mb-2 flex items-center gap-2">
               ...
               <!-- Slider -->
            <Slider v-else-if="field.type === 'slider'" :model-value="[field.min || 0]" :max="field.max" :min="field.min" :step="field.step" disabled />

              <!-- Date -->
              <Button v-else-if="field.type === 'date'" variant="outline" class="w-[240px] justify-start text-left font-normal text-muted-foreground" disabled>
                <CalendarIcon class="mr-2 h-4 w-4" />
                <span>{{ field.placeholder || '选择日期' }}</span>
              </Button>

              <!-- Combobox -->
              <Button v-else-if="field.type === 'combobox'" variant="outline" class="w-[240px] justify-between text-left font-normal text-muted-foreground" disabled>
            ...
            <div v-else-if="field.type === 'alert-dialog'" class="border rounded-md p-4 text-sm text-muted-foreground">
              确认弹窗：{{ field.description || '确认操作提示' }}
            </div>

            <!-- Drawer (placeholder) -->
            <div v-else-if="field.type === 'drawer'" class="border rounded-md p-4 text-sm text-muted-foreground">
              抽屉（{{ field.side || 'left' }}）：内容占位
            </div>

            <!-- Sheet (placeholder) -->
            <div v-else-if="field.type === 'sheet'" class="border rounded-md p-4 text-sm text-muted-foreground">
