...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!auth) {
      ...
      this.logger.debug(`[${req.url}] Authorization header 存在，长度: ${auth.length}`)
    }
   ...
   */
  private async tryRefreshExpiredToken(token: string, res: Response): Promise<string | null> {
    ...
    try {
      ...
      if (!decoded || typeof decoded !== 'object' || !('exp' in decoded) || !('sub' in decoded)) {
        return null
      }
      ...
      if (expiredSeconds > 0 && expiredSeconds <= GRACE_PERIOD_SECONDS) {
        this.logger.debug(`Token 已过期 ${expiredSeconds} 秒，在宽限期内，尝试刷新`)
   ...
   */
  private async checkAndRefreshToken(user: JwtPayload | undefined, res: Response): Promise<void> {
    if (!user?.exp) return
