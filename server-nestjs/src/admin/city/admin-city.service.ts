...
import type { QueryAdminCityDto, CreateCityDto, UpdateCityDto } from './dto/admin-city.dto'
...
export class AdminCityService {
  ...
  async remove(id: string) {
    ...
    if (journeyCount > 0) {
      throw new BusinessException(ErrorCode.OPERATION_DENIED, '该城市下存在文化之旅，无法删除')
    }
  ...
  async batchDelete(ids: string[]) {
    ...
    if (journeyCount > 0) {
      throw new BusinessException(ErrorCode.OPERATION_DENIED, '选中的城市下存在文化之旅，无法删除')
    }
