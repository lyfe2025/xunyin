...
export class JobExecutorService implements OnModuleInit, OnModuleDestroy {
  ...
  async addJob(task: JobTask): Promise<boolean> {
    ...
    try {
      ...
      if (task.status === '0') {
        cronJob.start()
        this.logger.log(`定时任务已启动: ${task.jobName} (${task.cronExpression})`, 'JobExecutor')
      }

      return true
    } catch (error) {
      this.logger.error(`添加定时任务失败: ${task.jobName}`, (error as Error).stack, 'JobExecutor')
      return false
    }
  }
  ...
  private async executeJob(task: JobTask) {
    ...
    try {
      ...
      this.logger.error(`任务执行失败: ${task.jobName}`, (error as Error).stack, 'JobExecutor')
    } finally {
