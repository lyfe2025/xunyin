...
export class DeptService {
  ...
  private async updateChildrenAncestors(
    ...
    try {
      ...
      this.logger.log(`Updated ancestors for ${children.length} child departments`, 'DeptService')
    } catch (error) {
      this.logger.error(`Failed to update children ancestors: ${error}`, 'DeptService')
      throw error
    }
  }
